 ...
 * $Rev: _ $
 * -------------------------------------------
 * $Workfile: (35) akkradm.ppr_AkkreditierungsbescheidService.sql $
 *
 */

if not exists ( select * from sysobjects where id = object_id('akkradm.ppr_AkkreditierungsbescheidService') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
begin
	exec( N'create procedure akkradm.ppr_AkkreditierungsbescheidService
	as begin
...
alter procedure akkradm.ppr_AkkreditierungsbescheidService
as
begin
--> 
	set nocount on;
...
---> Message is received
		...
		from akkradm.AkkrAkkreditierungsbescheidQueue;
...
----> in this place you can put more instructions for diferent actions handlers.... depending on the type of message
		...
		if @rc = 0 and @MESSAGETYPE = 'AkkrAkkreditierungsbescheidFreigegeben'
			begin

				exec sp_xml_preparedocument @IMessage output, @Message;
				select @EVENT_NAME = EVENT_NAME, @AKRB_ID = AKRB_ID
				from openxml (@IMessage, '/message', 2)
				with (EVENT_NAME UDT_CODE, AKRB_ID UDT_ID);

				if ( left(@EVENT_NAME,9) = 'akrb.anbo' )
				begin
					exec akkradm.pac_AKRB_Freigegeben @EVENT_NAME, @AKRB_ID;
					exec akkradm.pac_ANBO_Katalog @EVENT_NAME, @AKRB_ID;
					exec akkradm.pac_ANBO_Status @EVENT_NAME, @AKRB_ID;
				end

				if ( left(@EVENT_NAME,9) = 'akrb.abie' )
					exec akkradm.pac_AKRB_Freigegeben @EVENT_NAME, @AKRB_ID;
