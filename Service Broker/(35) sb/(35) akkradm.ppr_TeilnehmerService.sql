 ...
 * $Rev: _ $
 * -------------------------------------------
 * $Workfile: (35) akkradm.ppr_TeilnehmerService.sql $
 *
 */

if not exists ( select * from sysobjects where id = object_id('akkradm.ppr_TeilnehmerService') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
begin
	exec( N'create procedure akkradm.ppr_TeilnehmerService
	as begin
...
alter procedure akkradm.ppr_TeilnehmerService
as
begin
--> 
	set nocount on;
...
---> Message is received
		...
		from akkradm.AkkrTeilnehmerQueue;
...
----> in this place you can put more instructions for diferent actions handlers.... depending on the type of message
		...
		if @rc = 0 and @MESSAGETYPE = 'AkkrTeilnehmerAngemeldet'
			begin

				exec sp_xml_preparedocument @IMessage output, @Message;
				select @EVENT_NAME = EVENT_NAME, @TEIL_ID = TEIL_ID
				from openxml (@IMessage, '/message', 2)
				with (EVENT_NAME UDT_CODE, TEIL_ID UDT_ID);

				exec akkradm.pac_TEIL_Angemeldet @EVENT_NAME, @TEIL_ID;

			end

		if @rc = 0 and @MESSAGETYPE = 'AkkrTeilnehmerAbgesagt'
			begin
				...
				exec akkradm.pac_TEIL_Abgesagt @EVENT_NAME, @TEIL_ID;
