 ...
 * $Date: 2009-10-40 16:00:00 $
 * $Rev: _ $
 * -------------------------------------------
 * $Workfile: (35) akkradm.ppr_AngebotService.sql $
 *
 */

if not exists ( select * from sysobjects where id = object_id('akkradm.ppr_AngebotService') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
begin
	exec( N'create procedure akkradm.ppr_AngebotService
	as begin
...
alter procedure akkradm.ppr_AngebotService
as
begin
--> 
	set nocount on;
...
----> in this place you can put more instructions for diferent actions handlers.... depending on the type of message
		...
		if @rc = 0 and @MESSAGETYPE = 'AkkrAngebotOhneAntrag'
			begin

				exec sp_xml_preparedocument @IMessage output, @Message;
				select @EVENT_NAME = EVENT_NAME, @ANBO_ID = ANBO_ID
				from openxml (@IMessage, '/message', 2)
				with (EVENT_NAME UDT_CODE, ANBO_ID UDT_ID);

				exec akkradm.pac_ANBO_OhneAntrag @EVENT_NAME, @ANBO_ID;

			end

		if @rc = 0 and @MESSAGETYPE = 'AkkrAngebotZulangeAusgesetzt'
			begin
				...
				exec akkradm.pac_ANBO_ZulangeAusgesetzt @EVENT_NAME, @ANBO_ID;
