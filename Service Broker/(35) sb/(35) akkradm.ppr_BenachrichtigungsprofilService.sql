 ...
 * $Rev: _ $
 * -------------------------------------------
 * $Workfile: (35) akkradm.ppr_BenachrichtigungsprofilService.sql $
 *
 */

if not exists ( select * from sysobjects where id = object_id('akkradm.ppr_BenachrichtigungsprofilService') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
begin
	exec( N'create procedure akkradm.ppr_BenachrichtigungsprofilService
	as begin
...
alter procedure akkradm.ppr_BenachrichtigungsprofilService
as
begin
--> 
	set nocount on;
...
---> Message is received
		...
		from akkradm.AkkrBenachrichtigungsprofilQueue;
...
----> in this place you can put more instructions for diferent actions handlers.... depending on the type of message
		...
		if @rc = 0 and @MESSAGETYPE = 'AkkrMeldungNeu'
			begin

				exec sp_xml_preparedocument @IMessage output, @Message;
				select @EVENT_NAME = EVENT_NAME, @MLOG_ID = MLOG_ID, @BEPR_ID = BEPR_ID
				from openxml (@IMessage, '/message', 2)
				with (EVENT_NAME UDT_CODE, MLOG_ID UDT_ID, BEPR_ID UDT_ID);

				exec akkradm.pac_BEPR_Benachrichtigung @EVENT_NAME, @MLOG_ID, @BEPR_ID;
